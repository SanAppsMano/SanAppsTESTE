  ...
  btnSearch.addEventListener('click', async e => {
    ...
    try {
      const res = await fetchWithTimeout(
        ...
        { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ codigoDeBarras: code, latitude, longitude, raio: Number(selectedRadius), dias: 3 }) },
        60000
      );
      const json = await res.json();
      loading.classList.remove('active'); btnSearch.textContent = 'Pesquisar';
      if (res.status === 202) { errorMessageDiv.textContent = json.message || 'Busca iniciada em background.'; }
      else if (res.status === 404) { errorMessageDiv.textContent = json.error || 'Nenhum estabelecimento encontrado para esse código.'; }
