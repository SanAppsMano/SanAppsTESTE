...
window.addEventListener('DOMContentLoaded', () => {
  ...
  btnSearch.addEventListener('click', async () => {
    ...
    if (loc === 'gps') {
      try { const pos = await new Promise((res, rej) => navigator.geolocation.getCurrentPosition(res, rej)); lat = pos.coords.latitude; lng = pos.coords.longitude; } catch { loading.classList.remove('active'); return alert('Não foi possível obter localização.'); }
    } else {
    ...
    try {
      ...
      const productName = primeiro.dscProduto || 'Produto não identificado';
  ...
  btnDescSearch.addEventListener('click', async () => {
    ...
    try {
      const resp = await fetch('/api/searchDescricao', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      itens.forEach(entry => {
        ...
        li.addEventListener('click', () => { barcodeInput.value = entry.produto.gtin; descModal.classList.remove('active'); });
